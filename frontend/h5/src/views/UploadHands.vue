<template>
  <div class="min-h-screen bg-slate-900 text-white p-6 flex flex-col relative overflow-hidden">
    ...
    <div class="mt-8 pb-8 relative z-10">
      <button
        @click="nextStep"
        :disabled="!isReady || uploading || navigationLoading"
        class="w-full py-4 rounded-xl font-bold transition-all flex items-center justify-center gap-2"
        :class="isReady && !uploading && !navigationLoading
          ? 'bg-gradient-to-r from-amber-400 to-amber-600 text-slate-900 shadow-[0_0_20px_rgba(251,191,36,0.2)]' 
          : 'bg-slate-800 text-slate-500 cursor-not-allowed'"
      >
        <template v-if="uploading || navigationLoading">
          <svg class="animate-spin h-5 w-5" :class="navigationLoading ? 'text-slate-900' : 'text-slate-400'" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
            <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle>
            <path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path>
          </svg>
          {{ navigationLoading ? '同步镜像中...' : '正在上传手相...' }}
        </template>
        <template v-else>
